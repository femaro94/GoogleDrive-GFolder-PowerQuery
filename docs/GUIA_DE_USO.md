# Guia de uso: funções GFolder no Power BI e no Excel

## Introdução

Este guia explica, de forma detalhada, como instalar, configurar e usar as funções **GFolder** em **Power BI Desktop** e em **Excel com Power Query**. O objetivo é permitir que você cole a função em uma consulta nula, invoque a URL de uma pasta pública do Google Drive e transforme os arquivos encontrados em dados utilizáveis no ambiente Microsoft.

A lógica geral de uso é simples, mas há detalhes importantes de configuração. A função depende de uma pasta do Google Drive compartilhada publicamente, e o acesso normalmente deve ser feito com credencial **Anônima**. Sem isso, a consulta pode não carregar corretamente.

## Antes de começar

Antes de importar a função, verifique os seguintes pré-requisitos.
...
No **Power BI Desktop**, a forma mais segura de usar a função é criá-la como uma consulta separada e, depois, criar outra consulta para invocá-la. Isso deixa o projeto mais organizado e facilita manutenção futura.
...
Você pode usar outro nome, mas, se quiser reutilizar os exemplos exatamente como estão na documentação, é melhor manter esse nome.
...
Dependendo da sua versão do Excel, o caminho exato pode variar ligeiramente, mas a ideia é entrar no editor onde ficam as consultas.
...
### 3. A estrutura depende da página pública do Google Drive
...
### 4. Power BI Service não deve ser tratado como cenário garantido
...
Seguindo o passo a passo deste guia, você terá uma base sólida para instalar a função, validar o retorno e transformar os arquivos encontrados em tabelas e estruturas prontas para análise.
